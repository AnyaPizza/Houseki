...
public class FoundryBlockEntity extends BlockEntity implements ExtendedScreenHandlerFactory<BlockPos>, ImplementedInventory {
     ...
     * Creates a FoundryBlockEntity at the given position and initializes its PropertyDelegate
     * to expose progress, max progress, current (or fallback) fuel time, maximum fuel time, and crafting state.
     */
    ...
    public FoundryBlockEntity(BlockPos pos, BlockState state) {
        ...
        this.propertyDelegate = new PropertyDelegate() {
            /**
             * Retrieves a property value exposed by the block entity's property delegate for the given index.
             *
             * @param index the property index (0 = current progress, 1 = maximum progress, 2 = remaining fuel time or a fallback if no fuel, 3 = maximum fuel time, 4 = crafting flag as 1 when crafting and 0 otherwise)
             * @return the integer value of the requested property, or 0 for unknown indexes
             */
            @Override
            ...
            /**
             * Sets a property value in the block entity's property delegate based on the property index.
             *
             * @param index the property index: 0 = progress, 1 = maxProgress, 2 = fuelTime, 3 = maxFuelTime
            ...
            /**
             * Number of properties exposed by the PropertyDelegate.
             *
             * @return the number of properties (5)
             */
            @Override
            public int size() {
                return 5;
    ...
    /**
     * Determine the burn time in ticks provided by the given fuel item.
     *
     * @param fuel the ItemStack to evaluate; only coal is treated as valid fuel
     * @return the burn time in ticks for the fuel (`1600` for coal), or `0` if the item is not a valid fuel
     */
    ...
    /**
     * Provides the block entity's four-slot internal inventory list.
     *
     * @return the internal DefaultedList of ItemStack representing the inventory
     */
    @Override
    ...
    /**
     * Provides the block position used to align the opened screen with this block entity.
     *
    ...
    /**
     * Provides the display name for the foundry block's UI.
     *
     * @return a translatable Text with the key "gui.houseki.foundry"
     */
    @Override
    ...
    /**
     * Creates the screen handler for the foundry block UI.
     *
     * @return the FoundryScreenHandler bound to this block entity and its property delegate
     */
    ...
    /**
     * Spawns this block entity's contained items into the world at the given position and performs standard block-replacement cleanup.
     *
    ...
    /**
     * Writes this block entity's persistent state into the given write view.
     *
     * Stores the inventory contents and the foundry's runtime fields so they can be persisted:
     * progress, max progress, remaining fuel time, and maximum fuel time.
     *
     * @param view container used to record persistent data for this block entity
     */
    @Override
    ...
    /**
     * Reads the block entity's persisted inventory and state values from the given ReadView.
     *
     * This restores the inventory and the fields: `progress`, `maxProgress`, `fuelTime`, and `maxFuelTime`.
     *
     * @param view the source to read persisted data from
     */
    @Override
    ...
    /**
     * Performs a server-side tick update for the foundry block entity, advancing fuel and crafting progress and applying resulting state changes.
     *
     * <p>Detects input changes and updates the required progress, consumes fuel when needed, advances crafting progress while fuel is available, produces the crafted result when progress completes, updates the block's LIT property, and marks the block entity dirty if any relevant state changed.</p>
     *
     * @param world the world containing the block entity
     * @param pos the block position of the block entity
     * @param state the current block state at {@code pos}
     */
    ...
    /**
     * Update this block entity's `maxProgress` to the melt time of the current foundry recipe,
     * or to `FoundryRecipe.DEFAULT_MELT_TIME` if no matching recipe is present.
     */
    ...
    /**
     * Determines whether the current input can be processed into an output that fits into the output slot.
     *
     * @return `true` if a matching FoundryRecipe exists for the current input and its result can be inserted into the output slot, `false` otherwise.
     */
    ...
    /**
     * Check whether an ItemStack can be inserted into a specific inventory slot without exceeding stack limits or mixing different items.
     *
     * @param slot  target slot index
     * @param stack ItemStack to insert; an empty stack is considered insertable
     * @return      `true` if the slot can accept the stack (slot is empty or contains the same item/components and the combined count does not exceed the slot's max), `false` otherwise
     */
    ...
    /**
     * Applies the currently matched foundry recipe: inserts the recipe's main output into the output slot and consumes one item from the input slot.
     *
     * If no matching recipe exists, no changes are made.
     */
     ...
     *
     * @return an Optional containing the first matching RecipeEntry<FoundryRecipe>, or Optional.empty() if no match is found
     */
    ...
    /**
     * Provide the inventory slot indices that are accessible from the given block face.
     *
     * @param side the face of the block being accessed
     * @return an array of slot indices accessible from the specified side; when `side` is `Direction.DOWN`
     *         the array contains only `OUTPUT_SLOT`, otherwise it contains `INPUT_SLOT` and `FUEL_SLOT`
     */
    @Override
    ...
    /**
     * Determines whether the given ItemStack may be inserted into the specified slot from the given side.
     *
     * @param slot  the target slot index (e.g., INPUT_SLOT, FUEL_SLOT, OUTPUT_SLOT)
     * @param stack the ItemStack proposed for insertion
     * @param side  the block face from which insertion is attempted, or null if not applicable
     * @return `true` if the stack can be inserted into the specified slot, `false` otherwise
     */
    @Override
    ...
    /**
     * Determines whether an item may be extracted from the given slot from the specified side.
     *
     * @param slot  the slot index being accessed
     * @param stack the item stack attempted to be extracted
     * @param side  the face from which extraction is attempted
     * @return      `true` if the slot is the output slot, `false` otherwise
     */
    @Override
    ...
    /**
     * Creates a packet to synchronize this block entity's state to clients.
     *
     * @return the packet containing this block entity's update data, or `null` if no packet should be sent
     */
    @Nullable
    @Override
    ...
    /**
     * Determines whether the given player is close enough to interact with this block entity.
     *
     * @param player the player to check proximity for
     * @return `true` if the player is within 4.5 blocks of this block entity's position, `false` otherwise
     */
    @Override
    ...
    /**
     * Removes all items from this block entity's inventory and marks the block entity as modified so the change is persisted and synchronized.
     */
    @Override
    public void clear() {
        inventory.clear();
